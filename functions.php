...
// Theme functions.
// =============================================================================
define("MERCURIO_URL", get_template_directory_uri());
define("MERCURIO_PATH", get_template_directory());
require_once(MERCURIO_PATH . "/inc/class-tgm-plugin-activation.php");
add_theme_support('post-thumbnails');
function mercurio_register_required_plugins() {
	$plugins = array(
        array(
			'name'               => esc_html__('Hybrid Composer',"mercurio"),
			'slug'               => 'hybrid-composer',
			'source'             => MERCURIO_PATH . '/inc/hybrid-composer.zip',
			'required'           => true,
			'version'            => '',
...
add_action( 'tgmpa_register', 'mercurio_register_required_plugins' );
if (!isset( $content_width )) $content_width = 1200;
add_theme_support( 'automatic-feed-links' );
add_theme_support('title-tag');
function mercurio_enqueue_front_end_script() {
    if (!defined("HC_PLUGIN_PATH")) {
        wp_enqueue_style("bootstrap", MERCURIO_URL . "/css/bootstrap/css/bootstrap.css", array(), "1.0", "all");
        wp_enqueue_script("bootstrap",  MERCURIO_URL . '/css/bootstrap/js/bootstrap.min.js', array("jquery"), "1.0", true);
        wp_enqueue_script("hc_script",  MERCURIO_URL . '/inc/default.js', array(), "1.0",true);
        wp_enqueue_style("hc_style", MERCURIO_URL . "/style.css", array(), "1.0", "all");
        wp_enqueue_style("hc_icons", MERCURIO_URL . "/inc/icons/icons.css", array(), "1.0", "all");
        wp_enqueue_style("hc_css_skin", MERCURIO_URL . "/skin.css", array(), "1.0", "all");
    }
    if (is_singular() && comments_open() && (get_option('thread_comments') == 1)) wp_enqueue_script('comment-reply');
}
add_action('wp_enqueue_scripts', 'mercurio_enqueue_front_end_script');
...
<select data-hc-setting="sidebars" id="sidebars-menu" name="sidebars-menu">
    <option value="" <?php if ($sidebar == "") echo "selected" ?>><?php esc_attr_e("None","mercurio") ?></option>
    <option value="right" <?php if ($sidebar == "right") echo "selected" ?>><?php esc_attr_e("Right","mercurio") ?></option>
    <option value="left" <?php if ($sidebar == "left") echo "selected" ?>><?php esc_attr_e("Left","mercurio") ?></option>
    <option value="right-left" <?php if ($sidebar == "right-left") echo "selected" ?>><?php esc_attr_e("Right and left","mercurio") ?></option>
</select>
<?php
...
add_action('add_meta_boxes', 'mercurio_sidebar');

function mercurio_theme_add_editor_styles() {
    add_editor_style(MERCURIO_URL . "/css/tiny-mce.css");
}
add_action('admin_init', 'mercurio_theme_add_editor_styles');

//MENU
function mercurio_init_menus() {
    register_nav_menus(
          array(
            'header-menu' => esc_html__('Header Menu',"mercurio"),
            'footer-menu' => esc_html__('Footer Menu',"mercurio"),
            'extra-menu' => esc_html__('Top mini Menu',"mercurio")
          )
    );
}
add_action('after_setup_theme', 'mercurio_init_menus');

//WIDGETS
function mercurio_widgets_init() {
    register_sidebar( array(
        'name'          => esc_html__('Right Sidebar',"mercurio"),
        'id'            => 'right_side_bar',
        'description'   => esc_html__('Global sidebar for pages, enable it on single page.',"mercurio"),
    ...
    register_sidebar(array(
        'name'          => esc_html__('Left Sidebar',"mercurio"),
        'id'            => 'left_side_bar',
        'description'   => esc_html__('Global sidebar for pages, enable it on single page.',"mercurio"),
        'before_widget' => '<div class="list-group list-blog">',
        'after_widget'  => '</div>',
    ...
    if (class_exists('WooCommerce')) {
        register_sidebar(array(
           'name'          => esc_html__('Woocommerce Shop Sidebar Left',"mercurio"),
           'id'            => 'woocommerce_shop_left_side_bar',
           'description'   => esc_html__('Shop sidebar, enable it on Theme options > List Post Types',"mercurio"),
           'before_widget' => '<div class="list-group list-blog">',
           'after_widget'  => '</div>',
           'before_title'  => '<p class="list-group-item active">',
           'after_title'   => '</p>',
        ...
        register_sidebar(array(
          'name'          => esc_html__('Woocommerce Shop Sidebar Right',"mercurio"),
        ...
        register_sidebar(array(
          'name'          => esc_html__('Woocommerce Item Sidebar Left',"mercurio"),
          'id'            => 'woocommerce_single_left_side_bar',
          'description'   => esc_html__('Single product sidebar, enable it on Theme options > List Post Types',"mercurio"),
          'before_widget' => '<div class="list-group list-blog">',
          'after_widget'  => '</div>',
          'before_title'  => '<p class="list-group-item active">',
          'after_title'   => '</p>',
        ...
        register_sidebar(array(
           'name'          => esc_html__('Woocommerce Item Sidebar Right',"mercurio"),
...
add_action( 'widgets_init', 'mercurio_widgets_init' );

//MAIN CONTENT
function mercurio_get_post_info() {
    $txt = "";
    ...
    if ( ! empty( $categories ) ) {
        foreach( $categories as $category ) {
            $output .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" alt="' . esc_attr( sprintf( __("View all posts in %s","mercurio"), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>' . $separator;
        }
    ...
    <div class="header-base search-results-header">
        <div class="container">
            <div class="row">
                <div class="col-md-9">
                    <div class="title-base text-left">
                        <h1><?php echo esc_html__("Search results for ","mercurio") . get_search_query() ?></h1>
                        <p><?php esc_attr_e("Search results for ","mercurio"); echo '<b><i>' . get_search_query() . '</i></b>'; ?></p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="search-page-box">
                    <form role="search" method="get" id="searchform" class="navbar-form" action="<?php echo esc_url(home_url()) ?>">
                        <div class="input-group">
                            <input name="s" id="s" type="text" class="form-control" placeholder="<?php esc_attr_e("Search for ...","mercurio"); ?>" />
                            <span class="input-group-btn">
                                <input class="btn btn-default" type="submit" id="searchsubmit" value="<?php esc_attr_e("Go","mercurio"); ?>" />
                            </span>
                        </div>
                    </form>
                </div>
...
?>                         <h3>
            ...
            if (defined("HC_PLUGIN_PATH")) {
                echo hc_get_the_excerpt(get_the_excerpt());
                            ...
                            <a class="circle-button btn btn-sm" href="<?php echo esc_url($link) ?>">
                                <?php esc_attr_e("Read more","mercurio") ?>
                            </a>
                        </div>
                    </div>
                </div>
    ...
    } else echo "<h2 class='no-search-results'>" . esc_html__("No results found ...","mercurio") . "</h2><hr class='space row-500' />";
                ?>
            </div>
        </div>
    </div>
...
function mercurio_set_default_menu() {
...
function mercurio_default_blog() {
    global $wp_query;
    if (have_posts()) {
        while (have_posts()) {
            the_post(); 
   ...
   <div class="advs-box advs-box-side default-box <?php if (is_sticky()) echo "sticky-post"; ?>" data-anima="fade-left" data-trigger="hover">
    <div class="row">
        <div class="col-md-12">
            ...
            <?php echo mercurio_get_post_info(); ?>
    ...
    <ul class="pagination-sm pagination-grid pagination default-pagination">
        <li class="prev">
            <?php echo get_previous_posts_link(__("< Previous","mercurio")); ?>
        </li>
        <li class="next">
            <?php echo get_next_posts_link(__("Next >","mercurio"), $wp_query->max_num_pages ); ?>
        </li>
    </ul>
    <?php }
}
